...
import { Pencil, Trash2, Eye, EyeOff, PlusCircle } from 'lucide-react';
  ...
  const renderRow = (user: any, idx: number) => (
    <tr key={user.id || idx} className="hover:bg-gray-50 transition border-b last:border-none">
      ...
      <td className="px-6 py-4 whitespace-nowrap">
        <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${user.isDeleted ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800'}`}>{user.isDeleted ? 'Deleted' : 'Active'}</span>
      </td>
      <td className="px-6 py-4 whitespace-nowrap text-right">
        <div className="flex justify-end space-x-2">
          ...
          {canToggleStatus(user) && (
            <button
              title={user.isDeleted ? 'Activate' : 'Deactivate'}
              onClick={() => handleToggleStatus(user)}
              className={`p-2 rounded transition ${user.isDeleted ? 'hover:bg-green-100 text-green-600' : 'hover:bg-yellow-100 text-yellow-600'}`}
            >
              {user.isDeleted ? <Eye size={18} /> : <EyeOff size={18} />}
            </button>
          )}
          {canDelete(user) && (
            <button title="Delete" onClick={() => openDeleteModal(user)} className="p-2 rounded hover:bg-red-100 transition"><Trash2 size={18} className="text-red-600" /></button>
          )}
        </div>
      </td>
    </tr>
    ...
    <AdminDashboardLayout>
      ...
      {loading ? (
        ...
        <>
          ...
          <Modal isOpen={deleteModal.open} title="Delete Admin" onClose={() => setDeleteModal({ open: false, user: null })}>
            <div className="space-y-4">
              <p>Are you sure you want to delete this admin? This action can be undone by reactivating the user later.</p>
              {error && <div className="text-red-600 text-sm">{error}</div>}
              <div className="flex justify-end gap-2">
