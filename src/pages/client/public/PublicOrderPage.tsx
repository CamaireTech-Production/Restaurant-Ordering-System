...
import { Dish as MenuItem, Category, Restaurant, OrderItem, Order } from '../../../types';
...
const PublicOrderPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchRestaurantData = async () => {
      ...
      try {
        ...
        setCategories(categoriesData.sort((a, b) => (Number(a.order) || 0) - (Number(b.order) || 0)));
        // Fetch menu items
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      if (!restaurantId || !restaurant) throw new Error('Missing restaurant');
